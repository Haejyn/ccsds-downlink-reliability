...
| REQ-PN-01 | PN 랜덤화는 자기 역원이다 | `PseudorandomizerTests#Applying_the_randomizer_twice_returns_the_original`<br>`PseudorandomizerTests#Sequence_starts_with_all_ones_and_repeats_every_255_bytes` | 2/2 | ✅ 통과 |
...
| REQ-RS-03 | 정정 능력을 넘으면 실패를 선언한다. 오정정이 나는 경우에는 프레임 CRC 가 2차 방어선이 된다 | `ReedSolomonTests#Beyond_correction_capability_the_decoder_fails_or_miscorrects_and_crc_catches_it`<br>`ReedSolomonTests#Arbitrary_garbage_never_throws_and_is_reported_as_failure` | 2/2 | ✅ 통과 |
...
| REQ-ASM-03 | 마커 손상은 허용치까지 견디고(flywheel), 넘으면 다시 탐색한다 | `FrameSynchronizerTests#Damaged_markers_within_tolerance_keep_the_lock`<br>`FrameSynchronizerTests#Flywheel_keeps_emitting_through_missing_markers_then_gives_up`<br>`FrameSynchronizerTests#A_false_marker_is_dropped_when_the_next_one_does_not_follow` | 5/5 | ✅ 통과 |
